      ...
      <div className="px-4 py-4 pb-24 space-y-5">
          ...
          <div className="space-y-2">
            ...
            {showAuth && (
              <div className="bg-muted rounded-2xl p-4 space-y-2">
                <button
                  onClick={() => {
                    dispatch({ type: 'SET_USER', user: { name: 'Demo Student', email: 'student@ttu.edu', isGuest: false } });
                    setShowAuth(false);
                  }}
                  className="w-full flex items-center gap-3 bg-background rounded-xl p-3 active:scale-[0.98] transition-transform"
                >
          ...
          <div className="bg-card border border-border rounded-2xl overflow-hidden">
            ...
            >
              ...
              <div className={`w-11 h-6 rounded-full relative transition-colors ${state.notifications ? 'bg-primary' : 'bg-muted'}`}>
                <div className={`w-5 h-5 rounded-full bg-white absolute top-0.5 transition-transform ${state.notifications ? 'translate-x-[1.375rem]' : 'translate-x-0.5'}`} />
              </div>
            </button>
        ...
        {!isGuest && (
          <div className="bg-card border border-border rounded-2xl overflow-hidden">
            ...
            <button
              onClick={() => {
                if (window.confirm("Are you sure you want to delete your Lumisync account? This action is permanent and cannot be undone.")) {
                  dispatch({ type: 'SIGN_OUT' });
                  alert("Your account has been deleted successfully.");
                }
              }}
              className="w-full flex items-center gap-3 p-4 text-left border-t border-border active:bg-muted/50 transition-colors"
            >
        ...
        <div className="text-center py-4 font-sans">
          ...
          <p className="text-[10px] text-muted-foreground mt-1">© {SITE_FOUNDING_YEAR} Lumisync. All rights reserved.</p>
        </div>
      </div>
    </div>
  );
