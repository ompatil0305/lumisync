...
async def ingest_data():
    ...
    # Query already ingested chunk titles to make runs resumable
    existing_titles = set()
    try:
        from sqlalchemy import text
        db = vector_service.SessionLocal()
        result = db.execute(text("SELECT metadata->>'title' FROM ttu_knowledge_chunks")).all()
        existing_titles = {row[0] for row in result if row[0]}
        db.close()
        logger.info(f"Found {len(existing_titles)} already ingested chunks in database.")
    except Exception as e:
        logger.warning(f"Could not query existing chunks (might be first run): {e}")

    documents_to_insert = []

    for item in CAMPUS_DATA:
        if item['title'] in existing_titles:
            logger.info(f"Skipping already ingested chunk: {item['title']}")
            continue
